...
public interface ScoreRepository extends CrudRepository<ScoreCard, Long> {
     ...
     */
    @Query("SELECT NEW microservices.book.gamification.game.domain.LeaderBoardRow(s.userId, SUM(s.score)) " + "FROM ScoreCard s " + "GROUP BY s.userId ORDER BY SUM(s.score) DESC")
